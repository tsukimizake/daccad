...
/// 算術式用の代入（変数名 -> 整数値）
pub type ArithSubstitution = HashMap<String, i64>;
...
mod tests {
    use super::*;
    use crate::parse::{ArithOp, arith_expr, number, var};
    ...
    fn test_arith_expr_from_term() {
        let term = arith_expr(ArithOp::Add, v("X"), number(1));
    ...
    fn test_linear_simple_addition() {
        ...
        assert_eq!(state.get_value("X"), Some(5));
    }
    ...
    fn test_linear_simple_subtraction() {
        ...
        assert_eq!(state.get_value("X"), Some(10));
    }
    ...
    fn test_linear_complex_expression() {
        // 2 * X + 3 = 11 -> X = 4
        let state = solve(ArithEq::eq(ArithExpr::num(2) * x() + 3, 11));
        assert_eq!(state.get_value("X"), Some(4));
    }
    ...
    fn test_linear_nested_expression() {
        ...
        assert_eq!(state.get_value("X"), Some(3));
    }
    ...
    fn test_linear_negative_result() {
        ...
        assert_eq!(state.get_value("X"), Some(-7));
    }
    ...
    fn test_division_with_offset() {
        ...
        assert_eq!(state.get_value("X"), Some(11));
    }

    #[test]
    fn test_division_negative_divisor() {
        // X / -2 = 5 -> X = -10
        let state = solve(ArithEq::eq(x() / ArithExpr::num(-2), 5));
        assert_eq!(state.get_value("X"), Some(-10));
    }
    ...
    fn test_division_by_variable_unsolvable() {
        // 6 / X = 2 -> unsolvable (残る)
        let state = solve(ArithEq::eq(ArithExpr::num(6) / x(), 2));
        assert!(!state.has_error());
        assert!(!state.remaining_constraints().is_empty());
    }
